...
(seq
  (let
    (f1/58
       (function x/59
         (let (y/60 (makemutable 0 x/59))
           (seq
             (for i/61 0 to 10 (setfield_ptr 0 y/60 (+. (field 0 y/60) 1.)))
             (field 0 y/60)))))
    (setfield_imm 0 (global Code!) f1/58))
  (let
    (f2/62
...
camlCode__iter_65:
  i/8[%esi] := R/0[%eax]
  if i/8[%esi] >=s 23 goto L100
  {i/8[%esi]* env/9[%ebx]*}
  A/11[%ecx] := alloc 12
  [A/11[%ecx] + -4] := 2301
  A/12[%eax] := [env/9[%ebx] + 8]
  A/13[%eax] := [A/12[%eax]]
  R/7[%tos] := 1.
  R/7[%tos] := R/7[%tos] +f float64[A/13[%eax]]
  ...
  I/16[%eax] := i/8[%esi]
  I/16[%eax] := I/16[%eax] + 2
  tailcall "camlCode__iter_65" R/0[%eax] R/1[%ebx]
  L100:
  A/10[%eax] := 1
  reload retaddr
  return R/0[%eax]
...
camlCode__f2_62:
  ...
  [y/9[%ebx] + -4] := 1024
  [y/9[%ebx]] := x/8[%ecx]
  clos/10[%eax] := y/9[%ebx] + 8
  [clos/10[%eax] + -4] := 3319
  [clos/10[%eax]] := "camlCode__iter_65"
  [clos/10[%eax] + 4] := 3
  [clos/10[%eax] + 8] := y/9[%ebx]
  A/11[%eax] := [y/9[%ebx]]
  reload retaddr
  return R/0[%eax]
